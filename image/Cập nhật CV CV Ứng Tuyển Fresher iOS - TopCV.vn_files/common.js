...
function loadColor()
{
    ...
    $.each(builder.colors, function (index, value) {
        if (value == builder.color) {
            options.append($('<span class="color active" style="background-color:#'+value+'" data-color="'+value+'">' +
                '<i class="fa fa-check"></i>'+
                '</span>'));
        } else {
            options.append($('<span class="color" style="background-color:#'+value+'" data-color="'+value+'">' +
                '<i class="fa fa-check"></i>'+
                '</span>'));
        }
    });
...
function loadFont()
{
    ...
    options.html('<select name="font" id="font-selector" style="width: 120px;"></select>');
    $.each(builder.fonts, function (key, label) {
        if (key == builder.font) {
            options.find('select').append($('<option value="' + key + '" selected>' + label + '</option>'));
        } else {
            options.find('select').append($('<option value="' + key + '">' + label + '</option>'));
        }
    });
